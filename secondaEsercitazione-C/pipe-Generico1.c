...
{  
   ...
   int pidFiglio, status, ritorno; 	/* per wait padre */
	...
	while (read(piped[0], &Lun, sizeof(Lun)))	/* il padre per prima cosa deve leggere la lunghezza delle varie linee inviate dal figlio */
	{ 	
		/* ricevuta la lunghezza, il padre puo' andare a leggere la linea/stringa */
		read(piped[0], mess, Lun);	/* possiamo di evitare il controllo su questa read */

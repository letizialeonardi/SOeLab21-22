...
int main(int argc, char *argv[])
{
   ...
   int pidFiglio, status, ritorno;    /* variabili per wait */
  	...
  	if (argc < 3) 	/* non e' specificato, ma l'esercizio ha senso solo se sono passati almeno due nomi di file! */
  	...
  	for (i=0; i < N; i++)
  	{
    		...
    		{
  			/* commentiamo la stampa di debugger dei figli altrimenti si inteferisce con la stampa su standard ouput dei caratteri s critti dal padre 
			...
			*/
			...
			/* chiusura lati pipe non utilizzati */
      			...
      			close(p[1][0]);
      			close(p[i % 2][1]); /* chiude il lato di scrittura della pipe non usata: quindi se i e' pari  */
			...
			/* N.B. i processi di indice pari (ad esempio 0) leggono i file di indice dispari (ad esempio argv[1]) e quindi devono selezionare i caratteri alfabetici, mentre i processi di indice dispari (ad esempio 1) leggono i file di indice pari (ad esempio argv[2]) e quindi devono selezionare i caratteri numerici */ 
      			...
      			while(read(fdr, &ch, 1) > 0)	/* ogni figlio leggere via via tutti i caratteri dal proprio file associato */
      			{
      			...
      			exit(0); /* non viene specificato di tornare un valore particolare e quindi si torna 0 intendendo successo, per la semantica UNIX! */
  	...
  	close(p[1][1]);
  	/* dato che il testo non lo specifica si decide di continuare a leggere dalle due pipe anche se non si riesce a garantire l'alternanza in output a causa del termine dei processi pari o dei processi dispari; appena pero' finiscono entrambi i 'tipi' di processi il padre smette di leggere dalle pipe */
  	nr0 = read(p[0][0], &ch0, 1);
  	nr1 = read(p[1][0], &ch1, 1);
  	while ((nr0 != 0) || (nr1 != 0))
  	{
    		tot = tot + nr0 + nr1; /* il totale viene ricavato sommando via via i valori di ritorno delle read da pipe, ricordando che uno dei due puo' anche essere 0 */
